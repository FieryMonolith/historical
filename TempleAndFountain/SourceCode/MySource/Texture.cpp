 ...
 *   screening.  Search Jason Stredwick.
...
GLuint CTexture::TexName()
{
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_width, m_height, 0,
     GL_BGRA_EXT, GL_UNSIGNED_BYTE, m_image[0]);
...
void CTexture::SetSize(int p_x, int p_y)
{
   ...
   int usewidth = (m_width * 4 + (PADSIZE - 1)) / PADSIZE;
   usewidth *= PADSIZE;
...
bool CTexture::LoadFile(LPCTSTR lpszPathName)
{
   ...
   if(!file)
   {
     string errorMessage("Unable to open image file: ");
     errorMessage += lpszPathName;
         ...
         for(c=0;  c<m_width*4;  c++)
         {
           ...
           if((c + 1) % 4 == 1) {
             ...
             *row++ = r;
             ...
             /*** BLUE SCREEN ******************/
             /*** Jason Stredwick **************/
             // By default the else set the alpha value to 0xff, completely
             //   opaque.  Now, I check for <255, 93, 0> color value and if
             //   found will set alpha to 0x00, completely transparent.
             if(b == 255 && g == 93 && r == 0) {
               *row++ = 0x00;
             } else {
               *row++ = 0xff;
...
bool CTexture::ReadPPMFile(istream &file)
{
   ...
   for(int r=h-1; r>=0; r--)
   {
      ...
      for(int c=0;  c<w;  c++)
      {
         m_image[r][c * 4 + 3] = 0xff;
         file.read((char *)&m_image[r][c * 4 + 2], 1);
         file.read((char *)&m_image[r][c * 4 + 1], 1);
         file.read((char *)&m_image[r][c * 4 + 0], 1);
      }
   }
...
GLuint CTexture::MipTexName()
{
   ...
   gluBuild2DMipmaps(GL_TEXTURE_2D, 4, m_width, m_height, GL_BGR_EXT, GL_UNSIGNED_BYTE, m_image[0]);
