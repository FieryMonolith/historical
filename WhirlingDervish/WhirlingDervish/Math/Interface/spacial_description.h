...
public:
  ...
  Matrix4x4d InverseRotationMatrix(void) const {
    return Transpose(d_orientation);
  }
  void SetOrientation(const Vector3d &out, const Vector3d &up);

  const Matrix4x4d &PositionMatrix(void) const { return d_position; }
  Matrix4x4d &PositionMatrix(void) { return d_position; }
  Matrix4x4d InversePositionMatrix(void) const{
    return Matrix4x4d(1.0, 0.0, 0.0, -1.0*d_position.At(0,3),
