...
bool hfcPopulationStorage6::Reproduce(EnvironmentData &ed,
  ...
  replicateOut << "Generation: " << d_Generation << endl;
  ...
  */
  ...
  d_traitSelection = vector<unsigned>(5,0);
  ...
  while(allCreatures.size() < d_populationSize){
    ...
    if((d_fMax > 5) && (allCreatures.size() < (d_populationSize / 3)))
      targetFit = (d_fMax - 3) + (rng.GetUInt(3) + 1); //swap back for results in WD_4_...
      //targetFit = (d_fMax - 3) + ((rng.GetUInt(3) + 1) + d_fMin); 
    else
    ...
    replicateOut << "candiates.size(): " << candiates.size() << endl;
    if(candiates.size() > 0){
...
void hfcPopulationStorage6::getCandidates(unsigned targetTrait, unsigned targetFit, vector<CreatureInfo*> &candidates){
  unsigned upperBound = (unsigned)targetFit;
  unsigned count = 0;
  unsigned traitMax = 0;
